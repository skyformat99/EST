...
namespace MPL
{
	...
	struct rewrap<TNewName, TOldName<Ts...>> { using type = TNewName<Ts...>; };
	...
	struct map<F, typelist<Ts...>> { using type = typelist<F<Ts>...>; };
	...
	struct fliter<F, typelist<>> { using type = typelist<>; };
	...
	template<typename T>
	using size = rewrap_t<size_raw, T>;
	...
	template<typename... Ts>
	constexpr bool or_(Ts... vs) { return (... || vs); }

	template <typename T, typename Typelist>
	struct contain;

	template <typename T, typename... Ts>
	struct contain<T, typelist<Ts...>> { static constexpr auto value = or_(std::is_same_v<T, Ts>...); };
	...
	{
		using type = std::conditional_t<contain_v<T, typelist<Ts...>>,
	...
	struct intersection
	{
		template<typename T> using is_in_U = contain<T, U>;
	...
	{
		Func _fun;
		constexpr folder(Func fun) : _fun(fun) {} //无需在意拷贝，因为根本不会被真正求值
	...
	struct complete : std::false_type {};
	...
	{
		return (void)std::initializer_list<int>{ (f(std::get<indices>(std::forward<Tuple>(tuple))), 0)...};
	}
	...
	constexpr decltype(auto) for_tuple(Tuple&& tuple, F&& f)
	{
		...
		return for_tuple_impl(std::forward<Tuple>(tuple), std::forward<F>(f), Indices{});
	}
